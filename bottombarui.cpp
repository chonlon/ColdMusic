...
BottomBarUI::BottomBarUI(QWidget *parent)
    : QWidget(parent), ui(new Ui::BottomBarUI)
{
    ui->setupUi(this);
    ui->volume_slider->setValue(50);
    play_state_ = PLAYING;
    connect(ui->volume_slider, SIGNAL(sliderMoved(int)),
            SLOT(on_volume_slider_sliderMoved(int)));
    connect(ui->next_btn, SIGNAL(clicked(bool)), this,
            SIGNAL(nextbtnClicked()));
    connect(ui->play_btn, SIGNAL(clicked(bool)), this, SLOT(playpauseSong()));
    connect(ui->previous_btn, SIGNAL(clicked(bool)), this,
            SIGNAL(previousbtnClicked()));
    connect(ui->play_slider, SIGNAL(sliderMoved(int)), this,
            SIGNAL(playSliderMoved(int)));
    connect(ui->play_slider, SIGNAL(playSliderPressed(int)), this,
            SIGNAL(playSliderPressed(int)));
    connect(ui->play_slider, SIGNAL(sliderReleased()), this,
            SIGNAL(playSliderRelased()));
}
...
void BottomBarUI::playpauseSong()
{
    if (play_state_ == PLAYING) {
        ...
        ui->play_btn->setStyleSheet(
            "QPushButton{border-image: url(:/icon/resource/icon/play_normal.png);}\
        ...
        qDebug() << "pausing";
        ui->play_btn->setStyleSheet(
            "QPushButton{border-image: url(:/icon/resource/icon/pause_normal.png);}\
