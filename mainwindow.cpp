...
void MainWindow::initConnect()
{
    connect(player_, SIGNAL(positionChanged(qint64)),
			this, SLOT(update_SliderPosition(qint64)));
    connect(m_leftSide_bar_, SIGNAL(addedMusic(QStringList)),
			this, SLOT(update_PlayList(QStringList)));
    //connect(m_leftSideBar, SIGNAL(updatePlayList(QStringList)), this,SLOT(update_PlayListToShow(QStringList)));
    connect(m_bottom_bar_, SIGNAL(volumeSliderValueChanged(int)),
			this, SLOT(setPlayerVolume(int)));
    connect(m_bottom_bar_, SIGNAL(nextbtnClicked()),
			this, SLOT(setNextSong()));
    connect(m_bottom_bar_, SIGNAL(previousbtnClicked()),
			this, SLOT(setPreviousSong()));
    connect(m_bottom_bar_, SIGNAL(playSong()),
			this, SLOT(play()));
    connect(m_bottom_bar_, SIGNAL(pauseSong()),
			this, SLOT(pause()));

    //这里两个slot需要配合, 避免拖动slider的时候导致刺耳的声音
    connect(m_bottom_bar_, SIGNAL(playSliderMoved(int)), 
			this, SLOT(playSliderMoved(int)));
    connect(m_bottom_bar_, SIGNAL(playSliderRelased()),
			this, SLOT(playSliderReleased()));
    //slider被点击的时候
    connect(m_bottom_bar_, SIGNAL(playSliderPressed(int)),
			this, SLOT(playSliderPressed(int)));
    connect(m_bottom_bar_, SIGNAL(showPlayListBtnClicked()),
			this, SLOT(showPlayList()));
    connect(m_bottom_bar_, SIGNAL(volume_btnClicked()),
			this, SLOT(volume_btnClicked()));

    connect(this, SIGNAL(updatePlayList(const std::vector<SongInfro> &)),
            m_content_widget_, SLOT(updateMusicList(const std::vector<SongInfro> &)));
}

//static void getSongsInfor(const QStringList *list, std::vector<SongInfro>* playList, MainWindow* pw){
static void getSongsInfor(const QStringList *list,
                          std::vector<SongInfro> *playList)
{
    ...
    for (QString fileName : (*list)) {
        ...
        if (avformat_open_input(&fmt_ctx, fileName_const_char, nullptr,
                                nullptr)) //throw exception
            return;
...
{
    this->m_bottom_bar_->setSliderPosition(
        position, static_cast<qint64>(player_->getSongTotalTime()));
}
